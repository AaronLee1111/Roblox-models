...
![premium](contact_sheet_premium.png)
![brunch](contact_sheet_brunch.png)

Twenty-nine stylized, pastel, low-poly food props for a Japanese/Korean dessert café.
...
| Croffle | 4,150 | 1.53 × 1.53 × 0.61 | 5: Plate, CroffleWaffle, CroffleLayers, Cream, Blueberry | `.glb`, `.fbx` |
...
| PearlMochi ✦ | 7,346 | 1.90 × 1.20 × 0.80 | 6: Tray, Gold, Mochi_Cream, Mochi_Lilac, Mochi_Pink, Pearl | `.glb`, `.fbx` |
| GoldenMacaron ✦ | 6,424 | 1.44 × 1.44 × 1.17 | 6: ShellCream, GoldFilling, Gold, Pedestal, Pearl, Ruby | `.glb`, `.fbx` |
| MoonlightMochi ✦ | 4,496 | 1.64 × 1.64 × 0.76 | 5: NightPlate, MoonGold, Mochi_Navy, Mochi_Lilac, Mochi_Cream | `.glb`, `.fbx` |
| RoseCheesecake ✦ | 6,332 | 1.72 × 1.72 × 1.00 | 6: Cheesecake, Rose, Leaf, Pearl, Gold, Plate | `.glb`, `.fbx` |
| StrawberryPancakes | 6,342 | 1.53 × 1.53 × 1.57 | 8: Plate, PancakeTop, PancakeSide, Sauce, Cream, Strawberry, StrawberryCut, Leaf | `.glb`, `.fbx` |
| MatchaPancakes | 5,202 | 1.53 × 1.53 × 1.27 | 7: Plate, PancakeTop, PancakeSide, Sauce, Cream, RedBean, Shiratama | `.glb`, `.fbx` |
| ChocolatePancakes | 4,854 | 1.53 × 1.53 × 1.29 | 7: Plate, PancakeTop, PancakeSide, Sauce, Cream, DarkChocolate, Blueberry | `.glb`, `.fbx` |
| BelgianWaffle | 3,116 | 1.53 × 1.53 × 0.48 | 4: Plate, Waffle, WaffleEdge, Butter | `.glb`, `.fbx` |
| StrawberryWaffle | 5,690 | 1.53 × 1.53 × 0.97 | 7: Plate, Waffle, WaffleEdge, Cream, Strawberry, StrawberryCut, Leaf | `.glb`, `.fbx` |
| FrenchToast | 3,448 | 1.53 × 1.53 × 0.48 | 7: Plate, ToastFace, Crust, Butter, Strawberry, StrawberryCut, Blueberry | `.glb`, `.fbx` |
| HoneyToast | 2,472 | 1.53 × 1.53 × 0.46 | 7: Plate, ToastFace, Crust, Butter, Honeycomb, Wood, Honey | `.glb`, `.fbx` |
| ShibuyaToast | 5,858 | 1.53 × 1.53 × 1.27 | 10: Plate, ToastTop, Crust, CubeCrumb, Vanilla, Cream, Strawberry, StrawberryCut, Mint, Caramel | `.glb`, `.fbx` |
| Crepe | 6,894 | 0.72 × 0.73 × 1.55 | 7: Stand, Crepe, Sleeve, Cream, Strawberry, StrawberryCut, Leaf | `.glb`, `.fbx` |
| NutellaCrepe | 4,960 | 1.53 × 1.53 × 0.50 | 7: Plate, CrepeTop, CrepeEdge, Banana, Spread, Cream, Hazelnut | `.glb`, `.fbx` |

✦ marks the premium (rare) line.
...
New brunch items are variants of existing assets, not built from scratch.
They share geometry, the café plate and the material style:

| Family | Base | Shared module | Variants |
|---|---|---|---|
| Pancakes | Soufflé Pancakes' stack, plate and sauce-with-drips | `pancake_family.py` | Strawberry, Matcha, Chocolate |
| Waffles | Croffle's waffle grid (real ridges + pocket-aligned texture, pooled sauce and painted drizzle) | `waffle_family.py` | Belgian, Strawberry, and the Croffle itself |
| Toast | One bread builder (slice or brick loaf, crust sides, crumb faces) | `toast_family.py` | French, Honey, Shibuya |
| Crepes | Crepe texture, the Matcha Crepe Cake's layered-wedge loft as a folded crepe, and a ruffled cone | `crepe_family.py` | Crepe (Harajuku cone), Nutella Crepe |

## Premium line

The premium desserts show rarity through shape, colour and presentation, not
realistic reflections, so they still look special under flat Roblox lighting:

- The glossy satin look on the mochi and pearls is a cartoon highlight
  painted into the texture, so it doesn't depend on the lighting.
- Gold is a flat warm yellow with no metallic setting. It's used for rims,
  beaded borders, crowns and moons.
- Jewel-tone contrast: an amethyst lacquer tray, a ruby on the crown, and a
  deep navy night-sky plate against the pastel mochi.
- Presentation pieces: trays, a pedestal, a pearl necklace, a crown and a
  standing crescent moon, all as chunky real geometry.

Shared parts for this line are in `scripts/premium_parts.py`: mochi, painted
shine, pearls, gems, gold flakes, stars and a puffy crescent.
...
python3 RobloxDesserts/scripts/build_pearl_mochi.py
python3 RobloxDesserts/scripts/build_golden_macaron.py
python3 RobloxDesserts/scripts/build_moonlight_mochi.py
python3 RobloxDesserts/scripts/build_rose_cheesecake.py
python3 RobloxDesserts/scripts/build_strawberry_pancakes.py
python3 RobloxDesserts/scripts/build_matcha_pancakes.py
python3 RobloxDesserts/scripts/build_chocolate_pancakes.py
python3 RobloxDesserts/scripts/build_belgian_waffle.py
python3 RobloxDesserts/scripts/build_strawberry_waffle.py
python3 RobloxDesserts/scripts/build_french_toast.py
python3 RobloxDesserts/scripts/build_honey_toast.py
python3 RobloxDesserts/scripts/build_shibuya_toast.py
python3 RobloxDesserts/scripts/build_crepe.py
python3 RobloxDesserts/scripts/build_nutella_crepe.py
```
